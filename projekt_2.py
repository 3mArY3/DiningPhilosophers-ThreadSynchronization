...
def main():
    ...
    while sum(dining_philosophers.posilki) > 0:
        print("-" * (n * 5))
        print("".join(map(str, dining_philosophers.status)), " : ", str(dining_philosophers.status.count('  J  ')), " <--  Number of philosophers currently eating")
        print("".join(map(str, dining_philosophers.trzymanie)))
        print("".join("{:3d}  ".format(m) for m in dining_philosophers.posilki), " <--", str("Number of meals"))
